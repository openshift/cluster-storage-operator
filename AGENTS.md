...
- **CSO Operator RBAC** — CSO runs with fine-grained Role-based RBAC instead of `cluster-admin`:
  - **Cluster-scoped permissions** — `manifests/08_0001_cluster_storage_operator_storage_role.yaml` contains a ClusterRole with 5 rules for cluster-wide resources:
