...
const (
	...
	metricsCertSecretName                  = "volume-data-source-validator-serving-cert"
)
...
func (c *VolumeDataSourceValidatorStarter) createVolumeDataSourceValidatorManager(
	...
	staticAssets := []string{
		...
		"volumedatasourcevalidator/05_service.yaml",
	...
	monitoringCtrl := newMonitoringController(clients, c.eventRecorder, time.Minute*5)
	mgr = mgr.WithController(monitoringCtrl, 1)
...
func (c *VolumeDataSourceValidatorStarter) withReplacerHook() deploymentcontroller.ManifestHookFunc {
	return func(spec *operatorapi.OperatorSpec, deployment []byte) ([]byte, error) {
		...
		pairs := []string{
			...
			"${KUBE_RBAC_PROXY_IMAGE}", os.Getenv(kubeRBACProxyImage),
